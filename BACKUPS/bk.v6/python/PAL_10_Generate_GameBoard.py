...
class PAL_10_Generate_GameBoard(object):
	...
	def PAL_10_Determine_Build_Number(self):
		...
		return unicode(len(self.Build_No))
	...
	def PAL_10_Print_Backward_Row(self, start, end, outFile):
		...
		for square in reversed(range(start, end+1)):
			...
			if puzzle_link in os.listdir(self.PUZZLE_PAGES_DIR):
				if square == '100':
					outFile.write('\n          <a href="./main/puzzles/pages/'+puzzle_link+'" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(boxnum)+'-end" onclick="">'+unicode(square)+'</div></div></a>')
				else:
					outFile.write('\n          <a href="./main/puzzles/pages/'+puzzle_link+'" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(boxnum)+'" onclick="">'+unicode(square)+'</div></div></a>')
			else:
				if square == '100':
					outFile.write('\n          <a href="pagenotfound.html" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(boxnum)+'-end" onclick="">'+unicode(square)+'</div></div></a>')
				else:
					outFile.write('\n          <a href="pagenotfound.html" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(boxnum)+'" onclick="">'+unicode(square)+'</div></div></a>')
	...
	def PAL_10_Print_Forward_Row(self, start, end, outFile):
		...
		for square in range(start, end+1):
			...
			if puzzle_link in os.listdir(self.PUZZLE_PAGES_DIR):
				if square_print=='GO':
					outFile.write('\n          <a href="./main/puzzles/pages/'+puzzle_link+'" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(curr_boxnum)+'-start" onclick="">'+unicode(square_print)+'</div></div></a>')
				else:
					outFile.write('\n          <a href="./main/puzzles/pages/'+puzzle_link+'" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(curr_boxnum)+'" onclick="">'+unicode(square_print)+'</div></div></a>')
			else: 
				outFile.write('\n          <a href="pagenotfound.html" alt="'+unicode(square)+'"><div class="box"><div class="box-grid-'+unicode(curr_boxnum)+'" onclick="">'+unicode(square_print)+'</div></div></a>')
	...
	def PAL_10_Print_Body(self, outFile):
		...
		outFile.write('\n          <div class="tutorialbox" onclick="">Take the tutorial &rang;</div>')
		outFile.write('\n          <p class="small">&nbsp;</p>')
		outFile.write('\n          </a>')
		outFile.write('\n          <p class="small">&nbsp;</p>')
		outFile.write('\n          <p class="small">&nbsp;</p>')
		...
		for sentence in self.Updates_Info['$VERSION$\n']:
			Build_No = self.PAL_10_Determine_Build_Number() + ' (ALPHA)'
			outFile.write('\n        <p>'+sentence.strip('\n')+Build_No+'</p>')
		#outFile.write('\n        <p>&nbsp;</p>')
		outFile.write('\n        <hr>')
		...
		outFile.write('\n        <p>Copyright &copy; Pythons and Ladders 2016</p>')
		outFile.write('\n      </div>')
